...
class BaseDataset(pl.LightningDataModule):
    """
    Base Dataset All PE datasets should inherit from.
    """
    ...
    def __init__(
        ...
        device: str = "cpu",
        dec: Callable = Cosine,
        psi: Callable = Uniform,
        phi: Callable = Uniform,
    ...
    @property
    def strain_dim(self):
        """Number of dimensions in the strain tensor"""
    ...
    def background_fnames(self):
        ...
        background_dir = self.data_dir / "background"
        fnames = background_dir.glob("*.hdf5")
        return fnames

    def train_val_split(self) -> Sequence[str]:
        """
        Split background files into training and validation sets
        based on the minimum duration of the validation set
        """
    ...
    def transform(self, parameters: Dict[str, Tensor]):
        """Make any transforms to parameters before inference"""
        return self.parameter_transformer(parameters)

    def preprocess(self, X, parameters):
    ...
    def setup(self, stage: str) -> None:
        ...
        self.val_background = self.load_val_background()

        self._logger.info("Building validation dataset")
    ...
    def load_val_background(self):
        # TODO: generalize to multiple files
        background = []
        for fname in self.val_fnames[0]:
            data = []
            with h5py.File(fname, "r") as f:
                for ifo in self.hparams.ifos:
                    background = f[ifo][:]
                    data.append(torch.tensor(background))
    ...
    def build_transforms(self, sample_rate: float):
        ...
        self.projector = WaveformProjector(self.hparams.ifos, sample_rate)
    ...
    def on_after_batch_transfer(self, batch, _):
        ...
        if self.trainer.training:
            ...
            data, parameters = self.inject(X)

        elif self.trainer.validating or self.trainer.sanity_checking:
            [data], [parameters] = batch
    ...
    def sample_extrinsic(self, N: int):
        """
        Helper function for sampling extrinsic parameters
        used to project waveforms
        """
        dec = self.hparams.dec(N)
        phi = self.hparams.phi(N)
        psi = self.hparams.psi(N)
        return dec, phi, psi
    ...
    def inject(self, X):
        ...
        dec, phi, psi = self.sample_extrinsic(N)
        waveforms = self.projector(waveforms, dec, phi, psi)
        ...
        # whiten data and scale parameters
        X, parameters = self.preprocess(X, parameters)
    ...
    def val_dataloader(self):
        ...
        """
        background = self.val_background
        ...
        stride_size = int(self.hparams.val_stride * self.sample_rate)
        ...
        # works on 4D tensors. Add a shift corresponding to current rank
        # to add more diversity of background data in validation
        shift = self.sample_rate * self.rank
        background = background[
            :, : num_kernels * stride_size + kernel_size + shift
        ]
        background = background[..., shift:]
        background = torch.Tensor(background).view(1, self.num_ifos, 1, -1)
        background = unfold_windows(
            background, kernel_size, stride=stride_size
        )
